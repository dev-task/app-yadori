...
## PHASE 2: モバイルアプリの機能実装 (進行中)

### MOBILE-1: 認証・プロフィール機能
- [ ] **画面作成:** ログイン、新規登録、プロフィール表示・編集画面を作成する (`packages/ui` のコンポーネントを利用)
- [ ] **ロジック接続:** `packages/logic` のSupabase認証関数を呼び出し、認証機能を実装する

### MOBILE-2: レビュー閲覧・投稿機能
- [ ] **画面作成:** レビュー一覧、詳細、投稿フォーム画面を作成する
- [ ] **地図機能の実装:** `react-native-maps` (またはExpoの地図コンポーネント) とMapbox APIを連携させ、Web版と同様の地図からの住所選択機能を実装する
- [ ] **ロジック接続:** レビューデータの取得、投稿処理を実装する

### MOBILE-3: いいね・コメント機能
- [ ] **UI実装:** いいねボタン、コメントセクションをレビュー詳細画面に実装する
- [ ] **リアルタイム機能:** Supabase Realtimeを利用し、いいねやコメントがリアルタイムに反映されるようにする

## PHASE 3: プッシュ通知機能の実装

### PUSH-1: クライアントサイドの実装
- [ ] **権限の取得:** モバイルアプリ起動時に、ユーザーにプッシュ通知の許可を求めるダイアログを表示する
- [ ] **トークンの保存:** ユーザーが許可した場合、Expoから取得したプッシュトークンを、Supabaseの`USER_DEVICES`テーブルに保存する

### PUSH-2: サーバーサイドの実装 (Supabase Functions)
- [ ] **Functionの作成:** `likes`や`comments`テーブルに新しい行が追加されたことをトリガーに実行されるSupabase Functionを作成する
- [ ] **通知送信ロジック:** Function内で、投稿主の`user_id`を元に`USER_DEVICES`テーブルからプッシュトークンを取得し、Expo Push Serviceに通知内容を送信する処理を実装する

### PUSH-3: データベース拡張
- [ ] **USER_DEVICESテーブルの作成:** プッシュ通知トークンを管理するテーブルを作成
- [ ] **RLSポリシーの設定:** ユーザーが自分のデバイス情報のみアクセスできるようにセキュリティを設定
...
- **comments**: コメントデータ
- **likes**: いいね情報
- **user_devices**: プッシュ通知用デバイス情報（新規追加予定）
