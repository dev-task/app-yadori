/*
  ...
  1. Constraints
    - Add bio length constraint (500 characters max)
    - Add nickname length constraint (1-50 characters)

  2. Indexes
    - Add nickname search index
    - Add email search index

  3. Triggers
    - Ensure updated_at trigger function exists
    - Create/update profile update trigger
  ...
  5. Views
    ...
    - Include review count, likes received, comments made, average rating
*/
...
LEFT JOIN (
  SELECT 
    user_id,
    AVG(
      (CASE WHEN rating_location IS NOT NULL THEN rating_location ELSE 0 END +
       CASE WHEN rating_sunlight IS NOT NULL THEN rating_sunlight ELSE 0 END +
       CASE WHEN rating_soundproof IS NOT NULL THEN rating_soundproof ELSE 0 END +
       CASE WHEN rating_environment IS NOT NULL THEN rating_environment ELSE 0 END)::DECIMAL /
      NULLIF(
